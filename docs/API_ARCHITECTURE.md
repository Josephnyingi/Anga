...
ANGA uses a **Unified API** approach where all features are consolidated into a single, comprehensive API endpoint. The architecture supports both mobile and web applications with seamless API integration.
...
### Web App Integration
- **Same-origin proxy** via nginx to avoid CORS issues
- **API routes**: `/api/*` → `backend:8000/*`
- **Nginx configuration** in `apps/web/nginx.web.conf`

### API Configuration
```dart
// Mobile: Direct backend connection
