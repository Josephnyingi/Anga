...
class EnhancedWeatherCard extends StatelessWidget {
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (!isLoading && error == null)
                    _buildWeatherContent(context, isDarkMode),
                ],
              ),
            ),
          ),
  ...
  Widget _buildWeatherContent(BuildContext context, bool isDarkMode) {
    final temperature = weatherData['temperature'] ??
        ...
        weatherData['temperature_prediction'];
    final rainfall = weatherData['rain'] ??
        weatherData['rain_sum'] ??
          ...
          Row(
            children: [
              Icon(Icons.thermostat,
                  color: _getTemperatureColor(temperature), size: 24),
  ...
  Widget _buildWeatherDescription(dynamic temperature, dynamic rainfall) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: descriptionColor.withValues(alpha: 0.1),
        borderRadius: BorderRadius.circular(8),
        border: Border.all(color: descriptionColor.withValues(alpha: 0.3), width: 1),
      ),
      child: Row(
        children: [
          Icon(_getDescriptionIcon(description),
              color: descriptionColor, size: 16),
          const SizedBox(width: 8),
          Expanded(
            child: Text(
              description,
  ...
  IconData _getWeatherIcon() {
    ...
    if (rainfall != null && rainfall > 5) return Icons.umbrella;
    if (temperature != null) {
      final temp = temperature.toDouble();
