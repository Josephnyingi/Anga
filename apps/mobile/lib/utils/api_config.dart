...
class ApiConfig {
  ...
  static String _getPlatformSpecificUrl() {
    ...
    // Manual override for testing - set this to your PC's IP address
    const manualIpOverride = String.fromEnvironment('MANUAL_IP_OVERRIDE', defaultValue: '');
    if (manualIpOverride.isNotEmpty) {
      debugPrint("🔧 Using manual IP override: $manualIpOverride");
      return "http://$manualIpOverride:8000";
    }
  ...
  static bool _isAndroidEmulator() {
    try {
      ...
      // Method 5: Check if we're on a physical device by looking for specific indicators
      // Physical devices typically don't have these emulator-specific indicators
      final hasEmulatorIndicators = Platform.environment.keys.any((key) =>
        key.toLowerCase().contains('emulator') ||
        key.toLowerCase().contains('avd') ||
        key.toLowerCase().contains('sdk')
      );
      
      if (hasEmulatorIndicators) {
        debugPrint("Found emulator indicators, assuming emulator");
        return true;
      }
      
      // If we can't determine, assume it's a physical device for safety
      // (localhost is more likely to work on physical devices)
      debugPrint("No emulator indicators found, assuming physical device");
      return false;
    } catch (e) {
